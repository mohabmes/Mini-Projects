...
class VirtualTrader
{
	...
	/*
	* Fetches info for a specific stockcode from CNN Finance
	...
	*/
	...
	{
		$url = "https://money.cnn.com/quote/quote.html?symb=".$stockcode;
		$html = file_get_html($url);

		$stockname = $html->find('h1.wsod_fLeft')[0]->innertext;
		$stockname = substr($stockname, 0, strpos($stockname, '<'));

		$current_price = $html->find('td.wsod_last span')[0]->innertext;
		$change_price = $html->find('span.posData')[0]->innertext;
		$change_percent = $html->find('span.posData')[1]->innertext;



		$stockinfo['code'] = trim($stockcode); // Stock code name (ex: GOOG)
		$stockinfo['name'] = trim($stockname); // Stock Company Name (ex: Google Inc.)
		$stockinfo['price'] = trim($current_price); // Stock price
		$stockinfo['diff'] = trim($change_price); // Stock Difference
		$stockinfo['diff_perc'] = trim($change_percent); // Stock difference in percent
	...
	function GetStockInfoDB($stockcode)
	{
		...
		else
		{
			...
			else
			{
				...
				$stockinfo['diff'] = $stockinfo['diff'];
				$stockinfo['diff_perc'] = $stockinfo['diff_perc'];
	...
	*/
	...
	function UpdateBalance($username, $newbalance){
		//UPDATE `users` SET balance=14 WHERE id=
		$query2 = $this->mysqli->prepare("UPDATE users SET balance=? WHERE username=?");
		$query2->bind_param("ds", $newbalance, $username);
		$query2->execute();
		$query2->close();
	}
